...
def get_obj_det_model_Drive():
    ...
    from transformers import pipeline
    emo_model = pipeline("text-classification", model="matthewburke/korean_sentiment")
...
if user_input:
    ...
    with torch.no_grad():
        ...
        emotion = emo_model(user_input)
        if emotion[0] == "LABEL_0": # 부정
          if emotion[1] < 50: # 부정인 확률 (score)
            emoji = "😶"
          else:
            emoji = "😢"
        else: #긍정
          if emotion[1] < 50: # 긍정인 확률 (score)
            emoji = "🙂"
          else:
            emoji = "😄"
